#!/usr/bin/env bash
set -e

# Colors
R='\033[1;31m'
G='\033[1;32m'
Y='\033[1;33m'
W='\033[1;37m'
C='\033[1;36m'
N='\033[0m'

print_jishnu_logo() {
    echo -e "${C}"
    echo "========================================================================"
    echo "                       _ _     _                 "
    echo "                      | (_)   | |                "
    echo "                      | |_ ___| |__  _ __  _   _ "
    echo "                  _   | | / __| '_ \| '_ \| | | |"
    echo "                 | |__| | \__ \ | | | | | | |_| |"
    echo "                  \____/|_|___/_| |_|_| |_|\__,_|"
    echo "                                                  "
    echo "                    POWERED BY DENIZTECH          "
    echo "========================================================================"
    echo -e "${N}"
}

idx_setup() {
    clear
    print_jishnu_logo
    echo -e "${R}🔧 IDX TOOL SETUP${N}"
    
    cd "$HOME"
    rm -rf myapp flutter 2>/dev/null || true
    mkdir -p "$HOME/vps123/.idx"
    cd "$HOME/vps123/.idx"
    
    echo -e "${C}📝 Creating dev.nix configuration...${N}"
    cat <<EOF > dev.nix
{ pkgs, ... }: {
  channel = "stable-24.05";
  packages = with pkgs; [
    unzip openssh git qemu_kvm sudo cdrkit cloud-utils qemu
  ];
  idx.workspace = { onCreate = { }; onStart = { }; };
}
EOF
    echo -e "${G}✅ IDX TOOL SETUP COMPLETE!${N}"
    echo -e "${R}👉 Now click 'REBUILD ENVIRONMENT' in the bottom popup!${N}"
    read -p "Press Enter to return to menu..."
}

download_manager() {
    clear
    print_jishnu_logo
    INSTALL_DIR="$HOME/.deniztech-vm"
    mkdir -p "$INSTALL_DIR"
    mkdir -p "$HOME/vms"

    # BYPASS SUDO FOR IDX
    if [ -d "/etc/nix" ] || [ -n "${IDX_WORKSPACE_ID:-}" ]; then
        echo -e "${Y}ℹ️ Cloud detected. Skipping 'sudo apt'. Ensure Option 2 was run.${N}"
    else
        echo "📦 Installing dependencies..."
        sudo apt update && sudo apt install -y qemu-system qemu-utils cloud-image-utils wget lsof curl
    fi

    echo "🌐 Step 4/5: Downloading latest VM Manager from GitHub..."
    GITHUB_REPO="denizissidev/vm-manager"
    # THE GITHUB VERSION CHECKER GANG
    LATEST_RELEASE=$(curl -s "https://api.github.com/repos/$GITHUB_REPO/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' || echo "main")

    echo "ℹ️ Latest version: $LATEST_RELEASE"
    curl -sL "https://raw.githubusercontent.com/$GITHUB_REPO/main/vm-manager.sh" -o "$INSTALL_DIR/vm-manager.sh"
    
    sed -i 's/\r$//' "$INSTALL_DIR/vm-manager.sh"
    chmod +x "$INSTALL_DIR/vm-manager.sh"
    echo "$LATEST_RELEASE" > "$INSTALL_DIR/version.txt"

    if ! grep -q "alias vmmanager=" "$HOME/.bashrc" 2>/dev/null; then
        echo -e "\nalias vmmanager='$INSTALL_DIR/vm-manager.sh'" >> "$HOME/.bashrc"
    fi

    echo -e "${G}✅ Success! Run 'source ~/.bashrc' then 'vmmanager'${N}"
    read -p "Press Enter to return to menu..."
}

while true; do
    clear
    print_jishnu_logo
    echo -e "1) 📥 DOWNLOAD VM-MANAGER"
    echo -e "2) 🔧 INSTALL TOOL FOR GOOGLE IDX"
    echo -e "0) 🚪 EXIT"
    echo
    # Fix for piped bash: read from /dev/tty to ensure input works
    read -p "Select an option: " choice < /dev/tty

    case $choice in
        1) download_manager ;;
        2) idx_setup ;;
        0) exit 0 ;;
        *) echo -e "${R}Invalid option${N}"; sleep 1 ;;
    esac
done
